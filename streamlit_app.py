...
def instantiate_trained_pipeline():
    ...
    # Standardised weather tracking array definitions
    ...
    for _ in range(2500):
        ...
        w = np.random.choice(weathers, p=[0.6, 0.25, 0.15]) # Historical probability weights
        
        # Core conditional scaling calculation logic
        base = 35
        if d in days[:5] and (7 <= h <= 10 or 16 <= h <= 20): 
            base += 40 # Standard peak hour delays
            
        # Specific impact multipliers for weather configurations
        if w == 'Light Rain':
            base += 12
        elif w == 'Heavy Rainy':
            base += 28
...
# Dropdown menu to pick weather condition
...
if st.sidebar.button("🔮 Predict Congestion Level"):
    ...
    # Categorization Threshold Mapping
    if score_pred < 40:
        status, color = "🟢 Low Congestion", "green"
    elif score_pred < 75:
        status, color = "🟡 Moderate Congestion", "orange"
    ...
    # Dynamic 24-Hour Trend Chart Generation
    st.subheader("📈 24-Hour Congestion Trend Curve")
    hours_axis = list(range(24))
    trend_df = pd.DataFrame([{'Route': selected_route, 'DayOfWeek': selected_day, 'Hour': h, 'Weather': selected_weather} for h in hours_axis])
    trend_preds = pipeline.predict(trend_df)
